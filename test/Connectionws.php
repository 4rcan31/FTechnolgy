...
<head>
    ...
    <link href="<?php echo routePublic('vendor/bootstrap/css/bootstrap.min.css') ?>" rel="stylesheet">
</head>
<body>
    <div class="container"><br><br>
        <div class="d-grid gap-2">
            <h2>Click for the connect websocket</h2>
            <button class="btn btn-primary" type="button" id = 'btnConnect'>Connect (step 5)</button>
    ...
    connectBtn.addEventListener('click', function() { //Cuando aprete el boton de conectar, se conectara al servidor
        ...
        socket.onopen = function() {
            ...
            socket.addEventListener('message', (event) => {
               // addToConsole(event.data); //se imprime el mensaje del servidor en la pantalla del cliente (se espera un pong)
                if(JSON.parse(event.data).message == 'pong'){
                    addToConsole("La conexion a sido exitosa!");
                }
            });
        };
